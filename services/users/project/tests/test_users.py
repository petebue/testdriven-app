...
class TestUserService(BaseTestCase):
    ...
    def test_main_with_users(self):
        """
        Ensure the main route behave correctly when users have been added to
        the database
        """
        add_user('hannibal', 'hlecter@greattaste.com')
        add_user('pennywise', 'pennywise@thedancingclown.com')
        with self.client:
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'All Users', response.data)
            ...
            self.assertIn(b'pennywise', response.data)
    ...
    def test_main_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='hannibal', email='hlecter@greattaste.com'),
